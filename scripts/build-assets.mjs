...
await Promise.all(Object.entries(collectionColors).flatMap(([collection, color]) => mapIcons.map(async (name) => {
  ...
  await sharp(Buffer.from(pin)).png().toFile(resolve(mapPinDir, `${collection}-${name}.png`))
})))
...
console.log(`Built app icons, synced ${mapIcons.length} map icons, generated ${mapIcons.length * Object.keys(collectionColors).length} WebGL map pins, and bundled the MapLibre worker modules.`)
